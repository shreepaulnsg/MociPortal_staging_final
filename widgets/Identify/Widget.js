  ...
  function (declare, _WidgetsInTemplateMixin, BaseWidget, TabContainer, List, IdentifyTask, IdentifyParameters,
    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin], { /*jshint unused: false*/
      ...
      _initResultFormatString: function () {
        ...
        var tBold = false, tItalic = false, tUnder = false, tColorHex = "#231F20";
        var vBold = false, vItalic = false, vUnder = false, vColorHex = "#231F20";
...
//       console.info(this.resultFormatString );
      ...
      _initIdentifyLayers: function () {
        ...
        this.configIdentLayers = [];
        console.log(this.identifyLayer)
         
        if(this.oldOptions){
          this.identifyLayer.removeOption(lang.clone(this.oldOptions));
    
          this.identifyLayer.store = null;
          this.identifyLayer.set('value', '');
          this.identifyLayer._setDisplay("")
        } 
        for (var j = 0; j < this.map.layerIds.length; j++) {
          var layer = this.map.getLayer(this.map.layerIds[j]);
           
          if (layer.visible == true && layer.type == undefined) { 
            var visibleLayers = layer.visibleLayers;
            console.log(layer.arcgisProps.title) 
            for (let k = 0; k < layer.layerInfos.length; k++) { 
              const sublayer = layer.layerInfos[k]; 
              if (visibleLayers.includes(sublayer.id) && sublayer.name != "Industrial_Area_Bnd") { 

                var sublayerName = sublayer.name;
               // if(layer.layerInfos.length > 1){
                  sublayerName = layer.arcgisProps.title +" : "+sublayer.name
               // }
                option = {
                  url :layer.url,
                  id: sublayer.id,
...
//end comment
          ...
          var configLayers = this.map.webMapResponse.itemInfo.itemData?.operationalLayers;
          var title = identifyResult.layerName;
          console.log(this.map.webMapResponse)
          var obj = {};
          // if(sessionStorage.getItem("UserType")=="DeptViewer1"){
          //   obj = identifyResult.feature.attributes;
          //   }
          ...
          //   else {
            ...
            var selectedIndex = this.identifyLayer.get('value')
            var selectedIndex_layerUrl = this.configIdentLayers[selectedIndex].url;

          configLayers.forEach(function (layer) {
            if(layer.url == selectedIndex_layerUrl){
            //if (title.toLowerCase().replace(/\s+/g, '').includes(layer.title.toLowerCase().replace(/\s+/g, ''))) {
              if(layer.layers[0].popupInfo){
              layer.layers[0].popupInfo.fieldInfos.forEach(function (att) {
                if(att.visible){
                  var columnName = att.label == "PD Area"? att.label.replace(/\s+/g, '').toUpperCase() : att.label;
                  obj[columnName] = identifyResult.feature.attributes[columnName];
                }
              });
            } else{
              obj = identifyResult.feature.attributes;
...
//              console.info(content, rsltContent);
      ...
      onOpen: function () {
        ...
        on(this.btnRefresh, 'click', lang.hitch(this, function(){
          //alert("bindLayers s")
          this._initIdentifyLayers();  
        }));
