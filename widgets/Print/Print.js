...
define([
  ...
  return declare([BaseWidget, Evented], {
    ...
    maxNoOfCols: 25, // to store table columns
    ...
    setMapLayout: function (printTemplate) {
      ...
      printTemplate.layout = this.reportTemplate;
     ...
     */
    _createPrintMapParameters: function (mapParams, customTextElement) {
      ...
      if (mapParams.printTemplate) {
        ...
        printTemplate.layoutOptions = {
          ...
          // customTextElements: this.reportTitle == "Factory Report" ? [
          //   { "Factory Name": "Ram Constructions" },
          //   { "PIN No": "123" },
          //   { "Plot No": "23452" },
          //   { "License No": "64623" }
        ...
        printTemplate.preserveScale = false;
        printTemplate.hasCopyrightText = true;
     ...
     */
    print: function (reportTemplate, reportTitle, printData, logoSrc, customTextElement) {
      var a, b, userAgent;
        ...
        setTimeout(lang.hitch(this, function () {
          Window.withDoc(this._printWindow.document,
            lang.hitch(this, function () {
              ...
              //Set Report logo
              this._setReportLogo(logoSrc);
              //Set Report title
              this._setReportTitle(this.reportTitle);
              //Set Report data
              this._setReportData(printData, customTextElement);
              //Set foot notes
              // this._setFootNotes();
              //Set accessibility
     ...
     */
    _setReportData: function (printData, customTextElement) {
      ...
      if (htmlDataDiv) {
        array.forEach(printData, lang.hitch(this, (reportData) => {
          length++;
          ...
          if (reportData.type === "table") {
            dom.byId("reportHeader").style.display = "";
            // if (this.reportTitle == "Plot Allocation Report" && length == 1) {
            //   reportData.addPageBreak = false;
            // }
            // if (this.reportTitle == "Plot Allocation Report" && length == 2) {
            //   reportData.addPageBreak = true;
            // }
            if (reportData.addPageBreak) {
              domClass.add(domNode, "esriCTPageBreak");
            }
            this._formatAndRenderTables(domNode, reportData, length);
          } else if (reportData.type === "html") {
            this._renderHTMLData(domNode, reportData);
          } else if (reportData.type === "map") {
            dom.byId("reportHeader").style.display = "none";
            if (reportData.title) {
            ...
            this._executePrintTask(reportData, domNode, errorButtonNode, customTextElement);
     ...
     */
    _executePrintTask: function (mapParams, parentNode, errorNode, customTextElement) {
      var printParams;
      printParams = this._createPrintMapParameters(mapParams, customTextElement);
    ...
    _addDetailedSectionTitle: function (title, titleParent) {
      var titleNode;
      var sanitizedTitle = jimuUtils.sanitizeHTML(title ? title : '');
      titleNode = domConstruct.create("th", {
        "innerHTML": sanitizedTitle,
        "class": sanitizedTitle ? "esriCTSectionTitle" : ""
      }, titleParent);
      return titleNode;
     ...
     */
    _formatAndRenderTables: function (tableParentNode, reportData, length) {
      ...
      for (i = 0, j = tableInfo.cols.length; i < j; i += chunk) {
        ...
        this._renderTable(
          ...
          newTableInfo, reportData.data.showRowIndex, length);
        if (breakLoop) {
          break;
        }
      }
     ...
     */
    _renderTable: function (tableParentNode, tableInfo, showRowIndex, length) {
      var table, thead, tableBody, tableTitleRow, tableHeaderRow;
      if (this.reportTemplate == "Plot_Allocation" && length == 1) {
        tableInfo.title = this.nls.Psummaryreport; //"Summary Report";
        this._addSectionTitle(tableInfo.title, tableParentNode);
      }
      // if (this.reportTemplate == "Factory-Report" && length == 1) {
      //   tableInfo.title = "Plot Coordinates";
      //   this._addSectionTitle(tableInfo.title, tableParentNode);
      // }
      ...
      if (this.reportTemplate == "Plot_Allocation" && length == 2) {
        thead = domConstruct.create("tr", {}, table);
        //tableTitleRow = domConstruct.create("div", {}, thead);
        var title = this.nls.Pdetailedreport;//"Detailed Report";
        var titleNode;
        var sanitizedTitle = jimuUtils.sanitizeHTML(title ? title : '');
        titleNode = domConstruct.create("div", {
          "innerHTML": sanitizedTitle,
        ...
        //this._addDetailedSectionTitle(tableInfo.title, tableParentNode);
      }
      // if (this.reportTitle == "Factory Report" && length == 2) {
      //   thead = domConstruct.create("tr", {}, table);
      //   //tableTitleRow = domConstruct.create("div", {}, thead);
      //   var title = "Details";
      //   var titleNode;
      ...
      //     { "innerHTML": "fuck<BR>", style: { "width": "100px" } }, tableTitleRow);
