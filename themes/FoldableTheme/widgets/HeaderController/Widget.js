...
define([
  ...
  function(declare, lang, array, html, aspect, query, on, keys, Tooltip, Deferred, mouse,
    ...
    portalUrlUtils, utils, Message, PopupTileNodes,ArcGISDynamicMapServiceLayer,Query,QueryTask,InfoTemplate,
    ...
    var clazz = declare([BaseWidget, PoolControllerMixin], {
       ...
       init :function() {
        ...
        // var layer = new ArcGISDynamicMapServiceLayer("https://sampleserver1.arcgisonline.com/ArcGIS/rest/services/Specialty/ESRI_StateCityHighway_USA/MapServer");
        // _map.addLayer(layer);
        ...
        // _map.setExtent(layer.fullExtent); 
        ...
        dojo.connect(_map, "onClick", this.executeQueryTask);
        ...
        _infoTemplate = new InfoTemplate("${STATE_NAME}", "State Fips: ${STATE_FIPS}<br />Abbreviation: ${STATE_ABBR}<br />Area: ${AREA}");

       // _symbol = new SimpleFillSymbol(new SimpleFillSymbol.STYLE_SOLID, new SimpleLineSymbol(new SimpleLineSymbol.STYLE_DASHDOT, new dojo.Color([255,0,0]), 2), new dojo.Color([255,255,0,0.5]));
       _symbol = new SimpleFillSymbol(esri.symbol.SimpleFillSymbol.STYLE_SOLID, new SimpleLineSymbol(esri.symbol.SimpleLineSymbol.STYLE_DASHDOT, new dojo.Color([255,0,0]), 2), new dojo.Color([255,255,0,0.5]));
       ...
       executeQueryTask :function(evt) {
         ...
         //build query filter
        // _query = new Query();
        // _queryTask = new QueryTask("https://sampleserver1.arcgisonline.com/ArcGIS/rest/services/Specialty/ESRI_StateCityHighway_USA/MapServer/1");

        
        // _query.returnGeometry = true;
        // _query.outFields = ["STATE_NAME", "STATE_FIPS", "STATE_ABBR", "HYPERLINK", "AREA"];


        //onClick event returns the evt point where the user clicked on the map.
        //This is contains the mapPoint (esri.geometry.point) and the screenPoint (pixel xy where the user clicked).
        //set query geometry = to evt.mapPoint Geometry
        // _query.geometry = evt.mapPoint;

        //alert("query started")
        //Execute task and call showResults on completion

        //_queryTask.on("complete", this._queryTaskExecuteCompleteHandler);

        //_queryTask.on("error", this._showError);

        //_queryTask.execute(_query, lang.hitch(this, this._showResults));

        // _queryTask.execute(_query, function (featureSet) {
      ...
      _queryTaskExecuteCompleteHandler:function (compleated) {
        alert("compleated")
      ...
      postCreate: function() {
        ...
        //this.titleNode.innerHTML=this.nls.Title;
        ...
        if (this.appConfig && this.appConfig.logo) {
          if(this.nls.Title=="Industrial Estate Map Viewer"){
            this.logoNode.src = this.appConfig.logo;
          html.removeClass(this.logoNode, 'hide-logo');
          }
          else if(this.nls.Title=="خريطة المنطقة الصناعية"){
            this.logoNode.src= "images/app-logoArabic.png";
            html.removeClass(this.logoNode, 'hide-logo');
          }
          //html.removeClass(this.logoNode, 'hide-logo');
        } else {
        ...
        this.switchableElements.title.innerHTML =
          utils.sanitizeHTML(this.appConfig.title ? this.nls.Title : '');
      ...
      _showIconContent: function(iconConfig) {
        ...
        if(iconConfig.inPanel === false){
          this.widgetManager.loadWidget(iconConfig).then(lang.hitch(this, function(widget) {
            ...
            html.setStyle(widget.domNode, 'left', 101);
